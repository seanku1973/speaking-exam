...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: sessions, error } = await supabase
      .from("exam_sessions")
      .select(
        "id,recording_path,recording_size_bytes,recording_finished_at,audio_finished_at,created_at"
      )
      .not("recording_path", "is", null)
      .limit(5000);

    if (error) {
      throw new Error(`讀取錄音紀錄失敗：${error.message}`);
    }

    const expired = (sessions || []).filter((row: any) => {
      const reference =
        row.recording_finished_at ||
        row.audio_finished_at ||
        row.created_at;
    ...
    for (let i = 0; i < expired.length; i += 100) {
      ...
      if (paths.length > 0) {
        ...
        if (removal.error) {
          throw new Error(
            `刪除超過 60 天錄音失敗：${removal.error.message}`
          );
        }
      }
      ...
      for (const row of batch) {
        ...
        if (update.error) {
          throw new Error(
            `更新錄音清理狀態失敗：${update.error.message}`
          );
        }
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Daily recording cleanup failed.",
      },
      { status: 500 }
    );
  }
